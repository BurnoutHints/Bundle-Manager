...
public class Bundle : IBundle
{
	...
	private const uint headerLength = 0x2C;
	private const uint resourceEntryLength = 0x50;
	...
	private void UpdateHeader()
	{
		...
		// Delta bundles have no alignment anywhere.
		if ((header.flags & (uint)Flags.DeltaBundle) != 0)
			header.resourceEntriesOffset = headerLength;
		else
			header.resourceEntriesOffset = (uint)Serializer.Align(headerLength, 0x10);
	}
	...
	private void WriteResourceData(EndianAwareBinaryWriter writer)
	{
		...
		// Unlike Paradise, bundle ends are aligned here.
		if ((header.flags & (uint)Flags.DeltaBundle) == 0)
			writer.Align(0x80);
	}
