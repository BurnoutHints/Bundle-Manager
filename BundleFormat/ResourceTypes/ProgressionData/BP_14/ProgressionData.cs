...
internal class ProgressionData : ISerializableResourceType
{
	...
	{
		foreach (EventJunction junction in eventJunctions.Data!.Collection)
		{
	...
	private void UpdateJunctionPointer(InstancePointer<RaceEventData> eventPointer, ulong raceEventDataLength)
	{
		...
		RaceEventData eventEntry = events.Data!.Collection.First(e => e.id == eventId);
	...
	{
		foreach (RaceEventData eventData in events.Data!.Collection)
