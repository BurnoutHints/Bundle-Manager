...
internal class RaceEventData
{
	...
	internal enum OnlineGameMode : byte
	{
		Race,
		RoadRage,
		BurningHomeRun
	...
	public bool Crashbreaker
	{
		get => (flags & (uint)Flags.Crashbreaker) != 0;
		set
		{
			if (value)
				flags |= (uint)Flags.Crashbreaker;
			else
				flags &= (uint)~Flags.Crashbreaker;
		}
	}
	...
	public GameMode Mode { get => (GameMode)mode; set => mode = (byte)value; }
	public OnlineGameMode OnlineMode { get => (OnlineGameMode)onlineMode; set => onlineMode = (byte)value; }
